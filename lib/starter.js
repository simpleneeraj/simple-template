...
const createProject = async (projectName) => {
  ...
  try {
    ...
    console.log(chalk.greenBright("[ 1 / 3 ] → Copying project..."));
    console.log(chalk.greenBright("[ 2 / 3 ] → Fetching node_modules..."));
    await copyProjectFiles(projectName, template);
    await updatePackageJson(projectName);
    console.log(chalk.greenBright("[ 3 / 3 ] → Linking node_modules..."));
    console.log(
      chalk.yellowBright(
        "\u001b[2m───────────────────────────────────────────────────────────────\u001b[22m"
      )
...
const installNodeModules = async (destination, spinner) => {
  spinner.text = chalk.cyan("Install node_modules...\n");
...
const updateNodeModules = async (destination, spinner) => {
  spinner.text = chalk.cyan("Update node_modules...\n");
...
const dedupeNodeModules = async (destination, spinner) => {
  spinner.text = chalk.cyan("Dedupe node_modules...\n");
...
const createGitignore = async (destination, spinner) => {
  spinner.text = chalk.cyan("Create .gitignore...");
